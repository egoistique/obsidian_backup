...
Глава посвящена **обработке исключений (exception handling)**, а также вопросам, связанным с **управлением состоянием** программы при возникновении ошибок.

Процесс обработки исключения включает:

1. Определение, что считать ошибкой.
    
2. Определение момента возникновения ошибки.
    
3. Решение, как устранить ошибку.
    
4. Восстановление состояния системы до ошибки.
    

> 💬 Ошибки часто происходят в переходных состояниях системы, поэтому важно уметь **возвращать систему в корректное состояние**.

Автор отмечает, что обработка исключений — **слабое место CLR**, хотя Microsoft со временем внесла улучшения. Далее рассматриваются:
...
- обертки для исключений,
    
- неперехваченные исключения и т. п.
...
При проектировании типа (например, `FileStream`, `StringBuilder`) программист:

- задает **члены типа** — свойства, методы, события;
    
- выбирает **допустимые действия** с типом;
    
- и определяет, **в каких случаях следует генерировать исключение**.
...
- `from` или `to` равны `null`;
    
- счета недействительны;
    
- недостаточно средств на исходном счете;
    
- переполнение целевого счета;
    
- `amount` ≤ 0 или имеет более двух знаков после запятой.
    

Если возникает ошибка — метод должен **сгенерировать исключение**.  
Метод возвращает `void`, что означает:

- при успехе — просто завершается;
...
ООП делает код выразительным и читаемым:

`Boolean f = "Jeff".Substring(1, 1).ToUpper().EndsWith("E"); // true`

Однако операции могут завершиться с ошибкой.  
В .NET для этого существует механизм **обработки исключений (exception handling)**, поскольку многие элементы (конструкторы, свойства, события, перегрузки операторов и т. п.) **не могут возвращать коды ошибок**.

---

##### ⚠️ Внимание: частота ≠ исключение

Некоторые разработчики ошибочно считают, что исключения зависят от **частоты событий**.  
Пример: чтение файла до конца.  
Автор подчеркивает, что нельзя предугадать все случаи использования метода, и частота ошибок не должна определять, будет ли выбрасываться исключение.

---

#### ⚙️ Механика обработки исключений
...
{     
	try {         
		// Код, требующий очистки или восстановления     
	}     
	catch (InvalidOperationException) {         
		// Восстановление после InvalidOperationException     
	}     
	catch (IOException) {         
		// Восстановление после IOException     
	}     catch {         
		// Восстановление после остальных исключений         
...
> 💡 Обычно достаточно двух блоков: `try` + `finally` или `try` + `catch`.
...
- код, требующий **очистки** (`finally`),
    
- или код, который может **выбросить исключение** (`catch`).
    

Каждый `try` должен иметь хотя бы один `catch` или `finally`.

> ⚠️ **Совет:**  
> Объем кода внутри `try` зависит от управления состоянием.  
> Если разные операции требуют разной реакции на ошибки — создавайте отдельные блоки `try` для каждой.

---

#### 🪤 Блок catch

Блок `catch` выполняется при возникновении исключения.  
Если исключения нет — он **пропускается**, а поток переходит к `finally`.

#### 🧩 Тип исключения

Указывается в скобках после `catch`, например:

`catch (IOException)`

- Можно также использовать **универсальный `catch` без типа** — он эквивалентен `catch (System.Exception)`.
    

> 💬 В Visual Studio для просмотра текущего исключения добавляют переменную `$exception` в окно отладки.

#### 🔍 Порядок поиска catch

CLR ищет подходящий `catch` **сверху вниз**:

1. Более конкретные типы исключений.
    
2. Их базовые классы.
    
3. Общий `System.Exception`.
    

Если нужного обработчика нет, исключение становится **необработанным**.

#### 🧭 После нахождения обработчика:

CLR выполняет:

- все промежуточные блоки `finally`,
...
1. Повторно выбросить то же исключение (`throw;`);
    
2. Сгенерировать другое исключение;
    
3. Завершить блок `catch` без генерации.
    

> 💬 Можно присвоить имя объекту исключения:
> 
> `catch (IOException ex)`
> 
> и использовать `ex` для получения информации (например, трассировки стека).

---

##### 🧹 Блок finally

Блок `finally` выполняется **всегда**, даже если в `try` произошло исключение.
...
> ✅ Код в `finally` выполняется **всегда**, кроме случаев:
> 
> - принудительного завершения потока (`TerminateThread`),
>     
> - вызова `Environment.FailFast`.
>     

##### Рекомендации:

- Блок `finally` должен находиться **после всех catch**.
    
- Один `try` может иметь **только один finally**.
    
- Код в `catch` и `finally` должен быть **коротким и надежным** (1–2 строки).
    
- Исключения внутри `catch` или `finally` указывают на **серьёзные ошибки состояния**.
    

Если во время очистки возникает новое исключение, информация о первом теряется, и CLR завершает процесс для предотвращения повреждения состояния.

---

##### 🧾 Итог

Механизм обработки исключений в .NET основан на **структурной обработке исключений (SEH)**.  
Он обеспечивает:

- надёжную реакцию на ошибки,
...
> 💡 Главный принцип — **гарантировать корректное состояние программы даже при ошибках**.


### ⚙️ Класс `System.Exception`

#### 🧩 Общая характеристика

CLR позволяет генерировать исключения **экземплярами любого типа** — от `Int32` до `String`.  
Однако Microsoft решила ограничить разработчиков, чтобы обеспечить **совместимость языков**.

🔹 Для этого был создан базовый тип **`System.Exception`**.  
🔹 Только исключения этого типа и его **производных** считаются **CLS-совместимыми**.  
🔹 Компиляторы (в том числе C#) разрешают генерировать **только CLS-совместимые исключения**.

---

#### 📋 Основные свойства класса `System.Exception`

> Обычно к этим свойствам напрямую не обращаются —  
> их используют при отладке или анализе дампов памяти после сбоев.

|🧱 Свойство|🔓 Доступ|🧠 Тип|📝 Описание|
|---|---|---|---|
|**Message**|Только чтение|`String`|Текст с описанием причины исключения. Используется для журналов, содержит технические детали, полезные разработчикам.|
|**Data**|Только чтение|`IDictionary`|Коллекция пар «параметр–значение». Код может добавлять сведения перед генерацией исключения, а обработчик — использовать их.|
|**Source**|Чтение / запись|`String`|Имя сборки, сгенерировавшей исключение.|
|**StackTrace**|Только чтение|`String`|Трассировка вызовов методов, приведших к исключению — ключевая информация для отладки.|
|**TargetSite**|Только чтение|`MethodBase`|Имя метода, где возникло исключение.|
|**HelpLink**|Только чтение|`String`|Ссылка на справку (например, `file://C:\MyApp\Help.htm#MyExceptionHelp`). Используется редко из соображений безопасности.|
|**InnerException**|Только чтение|`Exception`|Указывает на внутреннее исключение, если текущее возникло во время обработки другого. Метод `GetBaseException()` возвращает самое первое исключение в цепочке.|
|**HResult**|Чтение / запись|`Int32`|32-битный код ошибки, используемый при взаимодействии управляемого и неуправляемого кода (например, COM API).|

---

#### 🧠 Свойство `StackTrace`

`StackTrace` — одно из самых полезных свойств при отладке.  
Оно показывает **цепочку вызовов методов**, ведущих к исключению.

🔸 При создании объекта `Exception` значение `StackTrace` = `null`.  
🔸 Когда исключение возникает, CLR записывает **место его появления**.  
🔸 При перехвате исключения — **место его обнаружения**.

Таким образом, при чтении `StackTrace` CLR **строит строку с именами всех методов**,  
от точки генерации до точки перехвата исключения.

---

#### ⚠️ Важно: влияние повторного выброса исключения

> При повторной генерации исключения возможна потеря информации о месте возникновения.

#### 🚫 Ошибка — повторное выбрасывание через переменную
...
➡️ В этом случае **CLR обнуляет начальную точку**,  
и анализ стек-трейса покажет неверное место возникновения.

#### ✅ Правильно — повторное выбрасывание без параметров

`catch (Exception e) {     ...     throw; // ✅ CLR сохраняет исходное место возникновения исключения }`

Инструменты анализа (например, **FxCop**) предупреждают о первой ошибке и не ругаются на второй.

---

#### 🧩 Альтернативный подход: контроль успешности блока

Чтобы избежать потери исходной информации, можно использовать флаг успешности:
...
Свойство `StackTrace` показывает **только методы до момента обработки исключения**,  
но не выше (то есть не весь стек вызовов).

Чтобы получить **полный стек**, используют тип:

##### 🧮 `System.Diagnostics.StackTrace`

Этот тип позволяет:

- управлять трассировкой стека программно;
    
- просматривать **кадры вызовов (stack frames)**;
    
- анализировать последовательность вызовов методов.
    

Конструкторы `StackTrace`:

- могут строить стек **до момента создания объекта**;
    
- или **на основе переданного объекта `Exception`**.
    

---

#### 🔍 Символы отладки и номера строк

Если CLR находит файлы символов (`.pdb`),  
тогда `StackTrace` (или `ToString()` в `System.Diagnostics.StackTrace`)  
включает:
...
> 💡 Это делает трассировку значительно более полезной при отладке.

---

#### 🧩 Почему могут отсутствовать имена методов

Иногда в трассировке не видно некоторых методов. Возможные причины:

1. **Отсутствие информации о точке вызова** — стек хранит только место возврата потока.
    
2. **Инлайнинг (встраивание)** — JIT-компилятор подставляет тело метода прямо в вызывающий код,  
...
Чтобы трассировка была полной, можно **отключить подстановку (inline)**:

#### 🔧 Варианты:

1. **Компилятор C#:**  
    Использовать флаг `/debug` — он добавляет в сборку атрибут  
    `System.Diagnostics.DebuggableAttribute`  
    с флагом `DisableOptimizations`.
...
```cs
using System; 
using System.Runtime.CompilerServices;  internal sealed class SomeType {     [MethodImpl(MethodImplOptions.NoInlining)]     public void SomeMethod() {         ...     } }
```

> 💬 Атрибут `[MethodImpl(MethodImplOptions.NoInlining)]`  
> запрещает JIT-компилятору выполнять подстановку как в **отладочной**,  
> так и в **рабочей конфигурации**.

---

#### 🧾 Итог

Класс `System.Exception` — **основа всей системы обработки ошибок в .NET**.  
Он предоставляет структуру для:

- хранения информации об ошибке,
    
- отслеживания её источника,
    
- безопасной передачи данных между уровнями приложения.
    

> 📚 Грамотное использование свойств `Exception` (особенно `StackTrace`, `InnerException`, `HResult`)  
